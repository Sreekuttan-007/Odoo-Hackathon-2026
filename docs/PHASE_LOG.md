...
- **Architecture** (`app/services/intelligence.py`): deterministic **evidence builder** → **source registry** → provider call → **claim validation** → brief.
  ...
  - Provider: Anthropic Claude Haiku via the existing `ANTHROPIC_API_KEY` infra (Phase 7B) — **no second provider stack**. Strict system prompt (10 rules), structured-JSON-only output.
...
- **Config**: `AI_PROVIDER` (default `"anthropic"`, optional) added alongside the existing optional `ANTHROPIC_API_KEY`. `.env.example` updated with placeholders.
- **Tests**: `tests/test_intelligence.py` — 17 tests, provider always mocked: evidence privacy (asserts no bank/id/contact/secret and no employee name in the packet JSON), fingerprint determinism, no-key/NOT_COMPUTED fallback, valid grounded brief, unknown-source drop, no-source drop, severity-tampering normalisation, numeric-hallucination drop, timeout/500/malformed fallback (parametrised), EMPLOYEE 403, HR_MANAGER 403, HR_PAYROLL_USER 200, **DB non-mutation** (snapshot rules/payruns/payslips/lines/contracts before & after 3 brief calls — byte-identical), and a full DRAFT→…→PAID regression with the brief called at each step. All 199 backend tests pass; `tsc -b` + `vite build` clean.
